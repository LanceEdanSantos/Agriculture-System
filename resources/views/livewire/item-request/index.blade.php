<div class="container mx-auto p-4">
    <div class="flex justify-between items-center mb-6">
        ...
        <button wire:click="create" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300">
            Create New Request
        </button>
    </div>
                ...
                @foreach (\App\Models\ItemRequest::whereIn('farm_id', \App\Models\Farm::whereHas('users', fn($q) => $q->where('user_id', Auth::id())->where('is_visible', true))->pluck('id'))->latest()->paginate(10) as $request)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 flex space-x-2">
                            <button wire:click="show({{ $request->id }})" class="text-blue-600 hover:underline">View</button>
                            @if ($request->status === 'pending')
                                <button wire:click="edit({{ $request->id }})" class="text-green-600 hover:underline">Edit</button>
                                <button data-modal-target="delete-modal-{{ $request->id }}" data-modal-toggle="delete-modal-{{ $request->id }}" class="text-red-600 hover:underline">Delete</button>
                            @endif
                        </td>
                    </tr>
                        ...
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                <div class="flex items-center justify-between p-4 border-b dark:border-gray-600">
                                    ...
                                    <button type="button" class="text-gray-400 hover:text-gray-900 dark:hover:text-white" data-modal-hide="delete-modal-{{ $request->id }}">
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex justify-end mt-4 space-x-2">
                                        <button wire:click="destroy({{ $request->id }})" data-modal-hide="delete-modal-{{ $request->id }}" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">Delete</button>
                                        <button data-modal-hide="delete-modal-{{ $request->id }}" class="bg-gray-300 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-400">Cancel</button>
                                    </div>
                                </div>
                            </div>
                        </div>
